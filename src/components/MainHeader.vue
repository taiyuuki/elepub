...
import { date_now, is_empty_string, is_not_empty_string, is_void, str_random } from '@taiyuuki/utils'
...
import { mdiAlertOutline, mdiCheckCircleOutline } from '@quasar/extras/mdi-v6'
...
const build = async () => {
    ...
    const outputIsNone = is_empty_string(output.value)
    ...
    images.forEach((image, i) => {
        ...
        const name = confoundingImage.value ? `*?${str_random(12, 16)}.${getFileExt(image.name)}` : image.name
        const xhtml = confoundingXhtml.value ? `*?${str_random(12, 16)}.xhtml` : `P${i + 1}.xhtml`
    ...
    meta.date = date_now('yyyy-MM-dd')
    const time = Date.now()
    generating.value = true
    meta.output = `${output.value}\\${meta.title}`
    if (is_not_empty_string(volume.value)) {
...
const beforeBuild = () => {
    if (is_empty_string(cover.value.path)) {
        if (images.length === 0) {
            $q.notify({
                message: '请添加图片',
                type: 'negative',
        ...
        const chain = $q.dialog({
            title: '提示',
            message: '没有选择封面，默认以第一张图作为封面，是否继续？',
            cancel: true,
        })
        chain.onOk(() => {
