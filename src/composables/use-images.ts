import { isEmptyObj, objectKeys, throttle } from '@taiyuuki/utils'
import { useOptions } from './use-options'

const { chapterMode, firstPage, pageCount, firstChapter } = useOptions()
...
export function clearContents() {
  const kyes = objectKeys(contents)
  kyes.forEach(key => {
    delete contents[key]
  })
}
